...
func Migrate(ctx context.Context, db *sql.DB) error {
	_, err := db.ExecContext(ctx, `CREATE TABLE IF NOT EXISTS agones_fleet_state (
...
func NewPostgres(ctx context.Context, db *sql.DB, namespace string) (*Postgres, error) {
	...
	_, err := db.ExecContext(ctx, `INSERT INTO agones_fleet_state(namespace,payload) VALUES($1,$2) ON CONFLICT(namespace) DO NOTHING`, namespace, b)
	if err != nil {
		return nil, err
	}
...
func (p *Postgres) View(ctx context.Context) (*State, error) {
	var raw []byte
	if err := p.DB.QueryRowContext(ctx, `SELECT payload FROM agones_fleet_state WHERE namespace=$1`, p.Namespace).Scan(&raw); err != nil {
		return nil, err
	}
	s := New()
...
func (p *Postgres) Update(ctx context.Context, fn func(*State) error) error {
	...
	defer tx.Rollback()
	var raw []byte
	if err = tx.QueryRowContext(ctx, `SELECT payload FROM agones_fleet_state WHERE namespace=$1 FOR UPDATE`, p.Namespace).Scan(&raw); err != nil {
		return err
	}
	s := New()
	if err = json.Unmarshal(raw, s); err != nil {
	...
	if _, err = tx.ExecContext(ctx, `UPDATE agones_fleet_state SET payload=$1, updated_at=now() WHERE namespace=$2`, raw, p.Namespace); err != nil {
		return err
	}
	return tx.Commit()
}

...
// Run with AGONES_FLEET_TEST_DATABASE_URL set, then go test -race ./internal/state.
...
func newPostgresFixture(t *testing.T) *postgresFixture {
	t.Helper()
	dsn := os.Getenv("AGONES_FLEET_TEST_DATABASE_URL")
	if dsn == "" {
		t.Skip("set AGONES_FLEET_TEST_DATABASE_URL to an isolated PostgreSQL database to run integration tests")
	}
	...
	t.Cleanup(func() {
		...
		for _, namespace := range fixture.namespaces {
			if _, err := db.ExecContext(ctx, `DELETE FROM agones_fleet_state WHERE namespace=$1`, namespace); err != nil {

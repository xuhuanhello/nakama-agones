...
func TestServerEnvironmentRejectsUnsafeOrAmbiguousInput(t *testing.T) {
	...
	cases := map[string]string{
		...
		"reserved":           `{"AGONES_FLEET_WORKER_ID":"fixture-sensitive-content"}`,
		"reserved lowercase": `{"agones_fleet_bootstrap_token":"fixture-sensitive-content"}`,
		"reserved mixed":     `{"Agones_Fleet_ADMISSION_KEY":"fixture-sensitive-content"}`,
		"empty name":         `{"":"x"}`,
		"digit first":        `{"9GAME":"x"}`,
...
func TestServerEnvironmentSizeBoundsAndProgrammaticValidation(t *testing.T) {
	...
	for name, value := range map[string]string{"AGONES_FLEET_CONTROL_URL": "https://other.invalid", "GAME_DATA": "\x00"} {
...
func TestFromEnvReadsServerEnvironment(t *testing.T) {
	for key, value := range map[string]string{
		"AGONES_FLEET_DATABASE_URL": "postgres://local.invalid/fleet", "AGONES_GAME_IMAGE": "game@sha256:" + strings.Repeat("a", 64),
		"AGONES_FLEET_CONTROL_URL": "https://control.invalid", "AGONES_FLEET_BUILD_HASH": "test-build", "AGONES_FLEET_REGION": "local",
		"AGONES_FLEET_MAX_ROOMS": "2", "AGONES_FLEET_MIN_INSTANCES": "0", "AGONES_FLEET_MAX_INSTANCES": "1",
		"AGONES_FLEET_ADMIN_TOKEN": "test-admin-token-at-least-32-characters", "AGONES_FLEET_SIGNING_KEY": encoded(testConfig().SigningKey),
		"AGONES_FLEET_SERVER_ENV_JSON": `{"GAME_PORT":"7770","GAME_LABEL":"a $ value"}`,
	} {
		t.Setenv(key, value)
	}
	cfg, err := FromEnv()
	...
	t.Setenv("AGONES_FLEET_SERVER_ENV_JSON", `{"AGONES_FLEET_CONTROL_URL":"https://other.invalid"}`)
...
type environmentProvider struct {
	*fakeProvider
	last provider.StartRequest
}

func (p *environmentProvider) Start(ctx context.Context, req provider.StartRequest) (provider.Instance, error) {
	p.last = req
...
func TestServerEnvironmentReachesProviderWithoutEnteringState(t *testing.T) {
	...
	provider := &environmentProvider{fakeProvider: &fakeProvider{instances: map[string]provider.Instance{}}}
	...
	if provider.last.EnvironmentVariables["AGONES_FLEET_GAME_PORT_NAME"] != cfg.PortName || provider.last.EnvironmentVariables["AGONES_FLEET_GAME_PORT"] != "7770" {
		t.Fatal("Agones port name/bind port did not reach game host")
	}
	if provider.last.EnvironmentVariables["AGONES_FLEET_CONTROL_URL"] != cfg.ControlURL || provider.last.EnvironmentVariables["AGONES_FLEET_MAX_ROOMS"] != "2" || provider.last.EnvironmentVariables["AGONES_FLEET_WORKER_ID"] == "" {
...
func TestServerEnvironmentFencesProfileChanges(t *testing.T) {
	...
	store := state.NewMemory()
	provider := &fakeProvider{instances: map[string]provider.Instance{}}
...
func TestEmptyServerEnvironmentProducesSameProfile(t *testing.T) {
	store := state.NewMemory()
	for _, environment := range []map[string]string{nil, {}} {
		cfg := testConfig()
		cfg.ServerEnvironment = environment
		if _, err := New(cfg, store, &fakeProvider{instances: map[string]provider.Instance{}}); err != nil {
			t.Fatal(err)
		}
	}
}

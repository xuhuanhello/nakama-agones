...
func Register(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, initializer runtime.Initializer, m *Manager) error {
	...
	logger.Info("Agones FleetManager registered; namespace=%s mode=%s rooms=%d", m.cfg.DeploymentID, m.cfg.Mode, m.cfg.MaxRooms)
	return nil
}
...
func registerPlayerHooks(initializer runtime.Initializer, m *Manager) error {
	...
	for _, route := range []struct {
		name   string
		resume bool
	}{{"agones_fleet_assignment_get_v1", false}, {"agones_fleet_resume_v1", true}} {
		resume := route.resume
	...
	if err := initializer.RegisterRpc("agones_fleet_assignment_cancel_v1", func(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, payload string) (string, error) {

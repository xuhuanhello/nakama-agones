...
func (m *Manager) HTTPHandler() http.Handler {
	...
	mux.HandleFunc("GET /agones/fleet/v1/admin/status", m.admin(func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("POST /agones/fleet/v1/admin/drain", m.admin(func(w http.ResponseWriter, r *http.Request) {
		var input struct {
			WorkerID string `json:"worker_id"`
		}
	...
	if m.cfg.Mode == "local" {
		mux.HandleFunc("POST /agones/fleet/v1/admin/allocate", m.admin(func(w http.ResponseWriter, r *http.Request) {
			var input struct {
				UserIDs    []string `json:"user_ids"`
...
func (m *Manager) bootstrapHTTP(w http.ResponseWriter, r *http.Request) {
	...
	err := m.store.Update(r.Context(), func(s *state.State) error {
		...
		if worker.BootID == "" && m.now().Unix()-worker.CreatedAt > m.cfg.LaunchTimeout {
			return ErrForbidden
		}
...
func (m *Manager) heartbeatHTTP(w http.ResponseWriter, r *http.Request) {
	...
	err := m.store.Update(r.Context(), func(s *state.State) error {
		...
		if input.Sequence > worker.LastSequence {
			...
			if worker.ProviderReady && worker.Host != "" && input.Ready && worker.State != "stopping" && worker.State != "unknown" && worker.State != "requested" && worker.State != "starting" {
				if worker.Draining {

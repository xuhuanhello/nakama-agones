...
func New(cfg Config, store state.Store, provider Provider) (*Manager, error) {
	...
	profileParts := []any{cfg.BuildHash, cfg.Region, cfg.MaxRooms, cfg.PortName, cfg.Kubernetes.Profile(), encoded(derive(cfg.SigningKey, "profile"))}
	...
	// This independent manager never shares a namespace fingerprint with another host adapter.
...
func (m *Manager) Assignment(ctx context.Context, user, allocationID string, resume bool) (*Assignment, error) {
	...
	now := m.now().Unix()
	// Ready denotes admission of NEW rooms. A drained host still serves existing
	// sessions until reconnection leases and result writes have completed.
	// The session-level checks below prevent this exception from creating seats.
	canResumeDraining := resume && w != nil && w.Draining && w.State == "draining"
	if w == nil || !w.ProviderReady || (!w.Ready && !canResumeDraining) || w.BootID == "" || w.Host == "" || now-w.LastHeartbeat > m.cfg.HeartbeatTimeout || (w.State != "ready" && w.State != "draining") {
		return nil, ErrBusy
	}

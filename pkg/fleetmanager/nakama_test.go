...
func playerHooks(t *testing.T, m *Manager) *playerHookInitializer {
	...
	if len(initializer.rpcs) != 3 || initializer.rpcs["agones_fleet_assignment_get_v1"] == nil || initializer.rpcs["agones_fleet_resume_v1"] == nil || initializer.rpcs["agones_fleet_assignment_cancel_v1"] == nil {
...
func TestAssignmentRPCProfileValidationPrecedesLookup(t *testing.T) {
	...
	for _, name := range []string{"agones_fleet_assignment_get_v1", "agones_fleet_resume_v1"} {
...
func TestManualCompatibilityVersionsRemainOpaqueStrings(t *testing.T) {
	for _, version := range []string{"dm-v1", "1", strings.Repeat("a", 64)} {
		t.Run(version, func(t *testing.T) {
			...
			_, err = hooks.rpcs["agones_fleet_assignment_get_v1"](ctx, nil, nil, nil, string(payload))
			assertPlayerError(t, err, 5, "assignment not found")
		})
	}
}

...
func (p *fakeProvider) Start(_ context.Context, req provider.StartRequest) (provider.Instance, error) {
	for _, existing := range p.instances {
		if existing.CustomData["worker_id"] == req.CustomData["worker_id"] {
			return existing, nil
		}
	}
	p.starts++
	i := provider.Instance{InstanceID: fmt.Sprintf("provider-%d", p.starts), Status: "running", CustomData: req.CustomData, NetworkPorts: []provider.Port{{Name: "game", Host: "127.0.0.1", Protocol: "udp", ExternalPort: 30000 + p.starts}}}
	i.TTL = p.ttl
	p.instances[i.InstanceID] = i
	if p.ambiguous {
		return provider.Instance{}, &provider.APIError{StatusCode: 503, OutcomeUnknown: true}
	}
	return i, nil
}
func (p *fakeProvider) Get(_ context.Context, id string) (provider.Instance, error) {
	i, ok := p.instances[id]
	if !ok {
		return i, &provider.APIError{StatusCode: 404}
	}
	return i, nil
}
func (p *fakeProvider) List(context.Context) ([]provider.Instance, error) {
	out := []provider.Instance{}
...
func testConfig() Config {
	return Config{DeploymentID: "test", BuildHash: "test-build", Region: "local", PortName: "game", MaxRooms: 2, MaxInstances: 3, Kubernetes: agones.Config{APIURL: "https://kubernetes.default.svc", TokenFile: "token", CAFile: "ca", Namespace: "agones-games", Pool: "default", DeploymentID: "test", GameImage: "game@sha256:" + strings.Repeat("a", 64), PortName: "game", GamePort: 7770}, ControlURL: "https://control.invalid", AdminToken: "test-admin-token-at-least-32-characters", SigningKey: bytes.Repeat([]byte{1}, 32), Mode: "production", Interval: time.Second, LaunchTimeout: 120, AllocationTimeout: 180, PrepareTimeout: 20, ReservationTTL: 60, ReconnectSeconds: 90, HeartbeatTimeout: 8, LostTimeout: 60, IdleSeconds: 600, ProviderPollSeconds: 10, MinLifetimeForAdmission: 1800, MaxQueueAgeSeconds: 2}
}
func setup(t *testing.T) *fixture {
	t.Helper()
	f := &fixture{t: t, store: state.NewMemory(), provider: &fakeProvider{instances: map[string]provider.Instance{}}, sequences: map[string]uint64{}}
...
func (f *fixture) bootstrap(worker string) string {
	f.t.Helper()
	status, body := f.request("/agones/fleet/v1/agent/bootstrap", "", BootstrapRequest{WorkerID: worker, BootID: "boot-" + worker, BuildHash: f.m.cfg.BuildHash, BootstrapToken: encoded(derive(f.m.cfg.SigningKey, "bootstrap:"+worker))})
	if status != 200 {
...
func (f *fixture) heartbeat(worker string, rooms []RoomReport, results []CommandResult, metrics state.Metrics) HeartbeatResponse {
	f.t.Helper()
	f.sequences[worker]++
	status, body := f.request("/agones/fleet/v1/agent/heartbeat", encoded(derive(f.m.cfg.SigningKey, "agent:"+worker+":boot-"+worker)), HeartbeatRequest{WorkerID: worker, BootID: "boot-" + worker, Sequence: f.sequences[worker], Ready: true, Rooms: rooms, CommandResults: results, Metrics: metrics})
	if status != 200 {
...
func TestHeartbeatReplayAndBootstrapFencing(t *testing.T) {
	...
	f.clock.Add(5)
	status, _ := f.request("/agones/fleet/v1/agent/heartbeat", encoded(derive(f.m.cfg.SigningKey, "agent:"+wid+":boot-"+wid)), HeartbeatRequest{WorkerID: wid, BootID: "boot-" + wid, Sequence: f.sequences[wid], Ready: true})
	if status != 200 || f.snapshot().Workers[wid].LastHeartbeat != before {
		t.Fatal("replayed sequence refreshed liveness")
	}
	status, _ = f.request("/agones/fleet/v1/agent/bootstrap", "", BootstrapRequest{WorkerID: wid, BootID: "different-process", BuildHash: f.m.cfg.BuildHash, BootstrapToken: encoded(derive(f.m.cfg.SigningKey, "bootstrap:"+wid))})
	if status != 403 {
		t.Fatal("new process reused old reservations")
	}
	status, _ = f.request("/agones/fleet/v1/agent/heartbeat", "wrong", HeartbeatRequest{WorkerID: wid, BootID: "boot-" + wid, Sequence: 100})
	if status != 403 {

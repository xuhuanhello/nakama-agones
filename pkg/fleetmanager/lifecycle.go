...
func (m *Manager) start(ctx context.Context, w *state.Worker) error {
	...
	return m.store.Update(ctx, func(s *state.State) error {
		...
		if callErr != nil {
			...
			conflict := errors.As(callErr, &apiError) && apiError.StatusCode == 409
			if provider.IsOutcomeUnknown(callErr) || conflict {
				current.State = "unknown"
				current.Error = "create_outcome_unknown"
				current.NextCheckAt = now + 1
				if conflict {
					current.Error = "create_identity_or_config_conflict"
					s.CreationBlockedReason = "provider_configuration_requires_review"
				}
			} else {
				current.State = "failed"
				current.Error = "provider_create_failed"
				if errors.As(callErr, &apiError) && (apiError.StatusCode == 400 || apiError.StatusCode == 401 || apiError.StatusCode == 403 || apiError.StatusCode == 404 || apiError.StatusCode == 422 || apiError.Reason == "token_file_unavailable" || apiError.Reason == "invalid_token_file") {
					s.CreationBlockedReason = "provider_configuration_requires_review"
				}
			}
...
func (m *Manager) startRequest(w *state.Worker) provider.StartRequest {
	...
	// Validation reserves AGONES_FLEET_* for the controller. Copy custom values into
...
func (m *Manager) providerLifetime(s *state.State, w *state.Worker, instance provider.Instance) {
	if instance.TTL <= 0 {
		return
	}
	start := w.CreatedAt
...
func (m *Manager) endpoint(instance provider.Instance) (string, int, error) {
...
func (m *Manager) owned(instance provider.Instance, w *state.Worker) bool {
...
func (m *Manager) reconcileUnknown(ctx context.Context, w *state.Worker) error {
	...
	if err == nil {
		...
		if len(matches) <= 1 {
			// Reissue exactly the same deterministic resource name and immutable Secret.
			// Kubernetes 409 is adopted only after identity/config validation. This also
			// recovers a timeout which happened before either POST reached the API.
			if err = m.renewLease(ctx); err != nil {
				return err
			}
			call, cancel = context.WithTimeout(ctx, 5*time.Second)
			var result provider.Instance
			result, err = m.provider.Start(call, m.startRequest(w))
			cancel()
			if err == nil {
				matches = []provider.Instance{result}
			}
		}
	}
	now := m.now().Unix()
	return m.store.Update(ctx, func(s *state.State) error {
		current := s.Workers[w.ID]
		if s.Leader != m.owner || current == nil || current.State != "unknown" {
			return nil
		}
		...
		if err != nil {
			...
			var apiError *provider.APIError
			if errors.As(err, &apiError) && !provider.IsOutcomeUnknown(err) && (apiError.StatusCode == 400 || apiError.StatusCode == 401 || apiError.StatusCode == 403 || apiError.StatusCode == 409 || apiError.StatusCode == 422) {
				s.CreationBlockedReason = "provider_configuration_requires_review"
			}
			return nil
		}
		if len(matches) == 1 {
			...
			m.providerLifetime(s, current, matches[0])
...
func (m *Manager) poll(ctx context.Context, w *state.Worker) error {
	...
	return m.store.Update(ctx, func(s *state.State) error {
		...
		if err != nil {
			...
			if errors.As(err, &apiError) && apiError.StatusCode == 409 && apiError.Reason == "resource_identity_mismatch" {
		...
		if !m.owned(result, w) || result.CustomData["build_hash"] != nil && result.CustomData["build_hash"] != m.cfg.BuildHash {
			lost(s, current, "provider_ownership_mismatch", now)
			return nil
		}
		m.providerLifetime(s, current, result)
		...
		if result.Status == "unhealthy" {
			lost(s, current, "provider_unhealthy", now)
			current.State = "stopping"
			current.Draining = false // Provider health already ended these allocations.
			current.NextCheckAt = now
			return nil
		}
		if result.Status != "running" {
			current.ProviderReady = false
			if current.State == "ready" || current.State == "suspect" {
...
func (m *Manager) notify(ctx context.Context) error {
	...
	for _, a := range s.Allocations {
		...
		for _, user := range a.UserIDs {
			if err = nk.NotificationSend(ctx, user, "agones_fleet_assignment", map[string]any{"allocation_id": a.ID, "revision": a.Revision, "state": a.State}, 1101, "", false); err != nil {
				success = false
			}
		}
		if success {

...
artifact_name="nakama-agones-$plugin_version-nakama-$NAKAMA_VERSION-linux-amd64"
...
if command -v sha256sum >/dev/null 2>&1; then
  sha256sum agones.so fleet-migrate fleet-console fleet-console-control compatibility.json build-metadata.json > SHA256SUMS
else
  shasum -a 256 agones.so fleet-migrate fleet-console fleet-console-control compatibility.json build-metadata.json > SHA256SUMS
fi
cd "$repo_dir/dist"
COPYFILE_DISABLE=1 tar -czf "$artifact_name.tar.gz" \
  "$artifact_name/agones.so" \
  "$artifact_name/fleet-migrate" \
  "$artifact_name/fleet-console" \
  "$artifact_name/fleet-console-control" \

...
if command -v sha256sum >/dev/null 2>&1; then
  sha256sum agones.so fleet-migrate compatibility.json build-metadata.json > SHA256SUMS
else
  shasum -a 256 agones.so fleet-migrate compatibility.json build-metadata.json > SHA256SUMS
fi
cd "$repo_dir/dist"

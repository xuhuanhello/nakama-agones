...
Open a game Pod's details and its game-process diagnostics to see all eight windows in three tables: client visual waiting, server ACK stages, and simulation queue/work. Each row shows P50/P95/P99, count, the 60-second window and sample age **as reported in the heartbeat**, not a live browser clock. Missing or empty samples stay unknown; exited instances show their last heartbeat only. The adjacent thread table displays the reported `simulation_workers` and `audit_workers`, independently of CPU policy.

The read-only alert reference uses `snapshot.alerts.config` from the displayed snapshot. When unavailable it says unknown, with no browser defaults substituted. The client-wait P95 and frame-P99 thresholds do not apply to the six server diagnosis windows. Showing a measurement above a threshold alone does not establish an active alert: the monitor also checks heartbeat freshness, sample count and hold duration.

Instance memory values have different sources: `metrics.memory_bytes` is supplied by the game host, while Pod memory sums container `usage.memory` returned by the Kubernetes Metrics API. Do not subtract them to calculate sidecar usage or interpret either as managed-heap size. The reference DM Linux host reads process `WorkingSet64`, with `/proc/self/status` `VmRSS` as its fallback; other game hosts must document their own measurement. An unavailable process memory report is shown as unknown.

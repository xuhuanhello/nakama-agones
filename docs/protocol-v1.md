...
All times are Unix seconds. All routes use JSON and require HTTPS outside the local local. Bodies are bounded to128KiB; command responses contain at most64 pending commands. Server credentials and admission tickets must not be logged.
...
| RPC | Payload | Result |
| --- | --- | --- |
| `agones_fleet_assignment_get_v1` | `{}` or `{"allocation_id":"...","build_hash":"game-build","region":"sea"}` | Own current allocation; signed admission fields only when assigned/active and eligible |
| `agones_fleet_resume_v1` | `{"allocation_id":"...","build_hash":"game-build","region":"sea"}` | Same owned seat, fresh nonce and short resume ticket within reconnect policy |
| `agones_fleet_assignment_cancel_v1` | `{"allocation_id":"..."}` | `{"accepted":true}`; cleanup completion must still be queried |
...
`build_hash` is the existing wire name for the application compatibility version; it is an opaque configured string, such as `dm-v1` or `1`, and need not be a content hash. `build_hash` and `region` are optional on assignment-get and resume. When present, each must be a string exactly matching the configured fleet pool. Empty strings and `null` are mismatches. Omitting either field preserves its previous behavior, so existing SDK requests remain supported. An incompatible build returns `agones_fleet_build_mismatch`; an incompatible region returns `agones_fleet_region_mismatch`. Both use Nakama runtime code9. Authentication is checked first, then these fields, then allocation ownership/state. Wrong JSON field types return invalid-payload code3.

Clients can call `agones_fleet_assignment_get_v1` with their build and region after login and before entering matchmaking. A compatible player without an allocation still receives the existing missing code5: it means no assignment exists and is not a profile mismatch. Handle the two stable mismatch messages explicitly and stop waiting/retrying with incompatible settings. This uses the existing RPC; no separate preflight endpoint is required.
...
`endpoint`, `boot_id` and `admission_token` are absent while waiting or terminal. Notifications use code1101, subject `agones_fleet_assignment`, and only `{allocation_id,revision,state}`. Treat them as hints and use the RPC for recovery. Tokens and endpoints are never broadcast to the whole match.

## Matchmaker queue admission

The plugin registers a `RegisterBeforeRt("MatchmakerAdd", ...)` hook. Every ticket must include `string_properties.build_hash` and `string_properties.region` matching the pool. Missing or mismatched values return the same `agones_fleet_build_mismatch` / `agones_fleet_region_mismatch` errors (runtime code9) before Nakama accepts the ticket. The matched hook repeats this validation as a final check.
...
The per-worker Kubernetes Secret supplies these environment variables per worker:

`AGONES_FLEET_WORKER_ID`, `AGONES_FLEET_BOOTSTRAP_TOKEN`, `AGONES_FLEET_ADMISSION_KEY`, `AGONES_FLEET_CONTROL_URL`, `AGONES_FLEET_BUILD_HASH`, `AGONES_FLEET_MAX_ROOMS`, `AGONES_FLEET_GAME_PORT`, `AGONES_FLEET_GAME_PORT_NAME`.

POST `/agones/fleet/v1/agent/bootstrap`:
...
POST `/agones/fleet/v1/agent/heartbeat`, `Authorization: Bearer <agent_token>`:
...
All use `Authorization: Bearer <AGONES_FLEET_ADMIN_TOKEN>` and should be access-restricted:

- GET `/agones/fleet/v1/admin/status`: state and safe error codes, no provider or admission secrets.
- POST `/agones/fleet/v1/admin/drain` with `{"worker_id":"..."}`: graceful drain of a ready/suspect/draining worker. Unknown/lost resources require separate investigation.
- POST `/agones/fleet/v1/admin/retry-creation`: clear a configuration failure circuit after correcting the provider configuration.
- POST `/agones/fleet/v1/admin/allocate`: only in explicit local mode, accepts `{user_ids:[...],request_key:"..."}` for tests. Not registered as an active handler in production.
